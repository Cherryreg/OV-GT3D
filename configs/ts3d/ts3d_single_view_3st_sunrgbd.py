...
model = dict(
    ...
    text_features_file='data/text_features/SUNRGBD_text_features_prompt.pt',
    train_cfg=dict(),
    test_cfg=dict(
...
data_root_val = 'data/sunrgbd/'

dataset_type = 'SUNRGBDDataset'
class_names = ('toilet', 'bed', 'chair', 'sofa', 'dresser', 'scanner', 'fridge',
                            'lamp', 'desk', 'table', 'stand', 'cabinet', 'counter', 'garbage_bin', 'bookshelf',
                            'pillow', 'microwave', 'sink', 'stool')
if train_stage == '1st':
    train_pipeline = [
    dict(
...
data = dict(
    ...
    val=dict(
        ...
        ann_file=data_root_val + 'sunrgbd_infos_val.pkl',   #scannet_infos_val.pkl_ov3det
    ...
    test=dict(
        ...
        modality=dict(use_camera=False, use_lidar=True),
        data_root=data_root_val,
        ann_file=data_root_val + 'sunrgbd_infos_val.pkl',#_ov3det
        pipeline=test_pipeline,
        classes=class_names,
        test_mode=True,
        box_type_3d='Depth'))
