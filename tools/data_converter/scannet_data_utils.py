...
class ScanNetData(object):
    ...
    def __init__(self, root_path, split='train', scannet200=False, save_path=None):
        ...
        if self.scannet200:
            ...
            # self.cat_ids = np.array([33,4,5,6,17,7,3,10,18,34,36,24,14,32,12,8,16,29,35,37])

            #OIS3D_setting
            #self.classes =['cabinet','bed','chair','sofa','table','door','counter','desk','sink','bathtub','window','bookshelf',
            #               'curtain','refridgerator','toilet','picture','shower']
            #self.cat_ids = np.array([3,4,5,6,7,8,12,14,34,36,9,10,16,24,33,11,38])

            ###B10N7_N7
            # self.classes = ['bed','chair','table','bookshelf','picture','sink','bathtub']
            # self.cat_ids = np.array([4,5,7,10,11,34,36])
        ...
        assert split in ['train', 'val', 'test']
        # split_file = osp.join('/home/ubuntu/xxw/SmallDet/mmdetection3d/dataset/meta_data_OV3Det',
        #                       f'scannetv2_{split}.txt')
        split_file = osp.join("/home/yifei/hyf/datasets_owp/scannet_all/meta_data",
                              f'scannetv2_{split}.txt')
        #split_file = osp.join("/home/yifei/hyf/datasets_owp/scannet_all/meta_data",
        #                      f'scannetv2_0274.txt')
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            #######rec mapping sv
            scannet25k_path = '/home/yifei/hyf/data/scannet_frames_25k'
            gt_from_image_file = osp.join(self.root_dir, 'gt_images', sample_idx + '_gtbox_image.npy')
            gt_from_image = np.load(gt_from_image_file)
            info['gt_from_image_mask'] = gt_from_image != '999999'
            intrinsics_file = osp.join(scannet25k_path, sample_idx, 'intrinsics_color.txt')
            mmcv.check_file_exist(intrinsics_file)
            info['intrinsics'] = np.loadtxt(intrinsics_file)
            extrinsics, img_paths = [], []
            for n in range(gt_from_image.shape[0]):    
                ##img
                img_path = osp.join(scannet25k_path, sample_idx, 'color', gt_from_image[n])
                # mmcv.check_file_exist(img_path)
                img_paths.append(img_path)
                if gt_from_image[n] != '999999':
                    ##extrinsics
                    extrinsic_file = osp.join(scannet25k_path, sample_idx, 'pose', gt_from_image[n][:-4] + '.txt')
                    mmcv.check_file_exist(extrinsic_file)
                    extrinsic = np.loadtxt(extrinsic_file)
                    extrinsics.append(extrinsic)
                else:
                    extrinsics.append(np.ones((4,4)))
            info['extrinsics'] = extrinsics
            info['img_paths'] = img_paths
            ######################
            ...
            if has_label:
                ...
                if annotations['gt_num'] != 0:
                    ...
                    #pdb.set_trace()
                    print(sample_idx, annotations['gt_num'], self.cat_ids2class, classes)
...
class ScanNetSegData(object):
    ...
    def __init__(self,
        ...
        self.all_ids = np.arange(256)  # all possible ids
        self.cat_ids = np.array([
