...
def export_one_scan(scan_name,
    ...
    if not test_mode:
        ...
        #把instance_label映射到正确的bbox上
        num_care_instances = aligned_bboxes.shape[0]
        all_instance_ids = np.arange(0, len(bbox_mask))
        care_instance_ids = np.arange(0, num_care_instances)
        instance_all2care = {id_formal + 1 : id_new for id_formal, id_new in zip(all_instance_ids[bbox_mask], care_instance_ids)}
        instance_all2care.update({id_not_care + 1 : -1 for id_not_care in all_instance_ids[~bbox_mask]})
        instance_all2care.update({0:-1})
        instance_labels = np.array([instance_all2care[instance_label] for instance_label in instance_labels])
    ...
    if not test_mode:
        if aligned_bboxes.shape[0] == 0:
            print('-----------------------------------------------------------------------------------------------------------------------')
        assert aligned_bboxes.shape[0] != 0
    ...
    if not test_mode:
        ...
        np.save(f'{output_filename_prefix}_axis_align_matrix.npy',
                axis_align_matrix)
        # np.save(f'{output_filename_prefix}_unaligned_bbox_sem_text.npy', unaligned_bboxes_sem)
        # np.save(f'{output_filename_prefix}_aligned_bbox_sem_text.npy', aligned_bboxes_sem)
...
def main():
    ...
    parser.add_argument(
        '--output_folder',
        default='./scannet/scannet_all/scannet_instance_data',
        help='output folder of the result.')
    parser.add_argument(
        '--train_scannet_dir', default="./scannet/scans/", help='scannet data directory.')
    parser.add_argument(
        '--test_scannet_dir',
        default="./scannet/scans_test/",
        help='scannet data directory.')
    parser.add_argument(
        '--label_map_file',
        default='./scannet/meta_data/scannetv2-labels.combined.tsv',
        help='The path of label map file.')
    parser.add_argument(
        '--train_scan_names_file',
        default="./scannet/meta_data/scannet_train.txt",
        help='The path of the file that stores the scan names.')
    parser.add_argument(
        '--test_scan_names_file',
        default="./scannet/meta_data/scannetv2_test.txt",
        help='The path of the file that stores the scan names.')
