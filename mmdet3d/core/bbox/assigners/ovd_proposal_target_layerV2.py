...
class ProposalTargetLayerV2(nn.Module):
    def __init__(self,
                 roi_per_image=128,
                 fg_ratio=0.5,
                 reg_fg_thresh=0.3,
        ...
        super(ProposalTargetLayerV2,self).__init__()
    ...
    def forward(self, batch_dict):
        ...
        batch_rois, batch_gt_of_rois, batch_gt_label_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, gt_assignment_idx = self.sample_rois_for_rcnn(
        ...
        targets_dict = {'rois': batch_rois, 'gt_of_rois': batch_gt_of_rois, 'gt_label_of_rois': batch_gt_label_of_rois,
                        ...
                        'batch_roi_ious': batch_roi_ious,
                        'gt_assignment_idx': gt_assignment_idx} # TODO: check class label
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        gt_boxes = batch_dict['gt_bboxes_3d_tensor']
        gt_labels = batch_dict['gt_labels_3d_owp']
        #########
        ...
        batch_gt_assignment = rois.new_zeros(batch_size, self.roi_per_image, dtype=torch.long)
        ...
        return batch_rois, batch_gt_of_rois, batch_gt_label_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, batch_gt_assignment
    ...
    def get_max_iou_with_same_class(rois, roi_labels, gt_boxes, gt_labels):
        ...
        for k in range(gt_labels.min().item(), gt_labels.max().item() + 1):
            ...
            if roi_mask.sum() > 0 and gt_mask.sum() > 0:
                ...
                ######
                iou3d = []
                for n in range(cur_gt.shape[0]):
                    gt = cur_gt[n].unsqueeze(0).expand(cur_roi.shape[0], 7)
                    ####iou
                    pred_to_targets_iou = diff_iou_rotated_3d(cur_roi.unsqueeze(0), gt.unsqueeze(0)).squeeze(0).detach()
                    iou3d.append(pred_to_targets_iou)
                iou3d = torch.stack(iou3d, dim=1)  # (M, N)
                #######
