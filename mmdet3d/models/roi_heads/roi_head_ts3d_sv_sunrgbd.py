...
class ROIHead_TS3D_SV_SUNRGBD(BaseModule):
    def __init__(self,
                in_channels,
                out_channels,
                voxel_size,
                ...
                train_cfg=None,
                test_cfg=None,):
        super(ROIHead_TS3D_SV_SUNRGBD, self).__init__(init_cfg=None)
        ...
        #             "There is no object in this image",]
        ##sunrgbd_N20
        self.eval_text = ['A photo of toilet', 'A photo of bed', 'A photo of chair', 'A photo of sofa', 'A photo of dresser', 'A photo of scanner', 'A photo of fridge', 'A photo of lamp', 'A photo of desk', 'A photo of table', 'A photo of stand', 'A photo of cabinet', 'A photo of counter', 'A photo of garbage_bin', 'A photo of bookshelf', 'A photo of pillow', 'A photo of microwave', 'A photo of sink', 'A photo of stool',
                    'A photo of wall',
                    'A photo of floor',
                    "An unclear image",
                    "A photo of background",
    ...
    def _forward_2st(self, batch_dict):
        ...
        bbox_maxplooing_feats, _ = self.maxpooling_bbox_feat_v2(seg_feats, second_pred_bboxes, img_metas)
        ...
        #######
        ##norm
        pred_bboxes_feats_clip_header = pred_bboxes_feats_clip_header / pred_bboxes_feats_clip_header.norm(dim=1,keepdim=True)
        ...
        img_calib = batch_dict['img_calib']
        extrinsics = img_calib['calib_Rtilt']
        intrinsics = img_calib['calib_K']
        images = img_calib['image']
        patches = []
        for bs in range(len(pred_bboxes_corners)):
            ...
            for n in range(pred_bboxes_corners[bs].shape[0]):
                ...
                pred_bbox_corner_2d = self.proj_pointcloud_into_image(pred_bbox_corner_3d, cur_bs_extrinsics, cur_bs_intrinsics)
                pred_bbox_corner_aug = pred_bboxes_corners_aug[bs][n]
                pred_bbox = second_pred_bboxes[bs][n]
                patch, valid_3d_bbox = self.crop_per_patch(cur_bs_images, pred_bbox_corner_2d, pred_bbox_corner_aug, cur_bs_second_points, pred_bbox)
    ...
    def forward_test_2st(self,batch_dict):
        ...
        bbox_maxplooing_feats, masks = self.maxpooling_bbox_feat_v2(seg_feats, batch_dict['rois_ovd'], img_metas)
        bbox_maxplooing_feats = torch.cat(bbox_maxplooing_feats, dim=0)
        pred_bboxes_feats_clip_header =  pred_bboxes_feats_clip_header + bbox_maxplooing_feats
        # pred_bboxes_feats_clip_header =  bbox_maxplooing_feats
        #######
        ##norm
        pred_bboxes_feats_clip_header = pred_bboxes_feats_clip_header / pred_bboxes_feats_clip_header.norm(dim=1,keepdim=True)
        ######
        ...
        bbox_list = self._get_bboxesV2(bbox_preds, pc_sem_cls_prob, box_score_preds, img_metas, masks)
        return bbox_list
    ...
    def forward_test_3st(self,batch_dict):
        ...
        results_nms = self.get_boxes_3st(batch_dict, img_metas) 
        #####obj
        batch_dict['rois_ovd'] = [results_nms[0][0]]
        pred_bboxes_feats_clip_header = self.roi_grid_pool_ovd(batch_dict)
        ##res
        seg_feats = batch_dict['semantic_feat']
        bbox_maxplooing_feats, _ = self.maxpooling_bbox_feat_v2(seg_feats, batch_dict['rois_ovd'], img_metas)
        bbox_maxplooing_feats = torch.cat(bbox_maxplooing_feats, dim=0)
        pred_bboxes_feats_clip_header =  pred_bboxes_feats_clip_header + bbox_maxplooing_feats
        #######
        ##norm
    ...
    def _get_bboxesV2(self, bbox_preds, cls_preds,  box_score, img_metas, masks = None):
        results = []
        ...
        return results
    def _get_bboxes_singleV2(self, bbox_preds, cls_preds, box_score, img_meta, mask=None):

        bbox_preds = bbox_preds
        scores = cls_preds * box_score
        if mask != None:
            boxes_ap, scores_ap, labels_ap = self._nms_openvoc(bbox_preds[mask], scores[mask], 0.0, 0.5, img_meta)
        else:
    ...
    def maxpooling_bbox_feat_v2(self, seg_feats, bboxes, img_metas):
        ...
        for i in range(len(img_metas)):
            ...
            rois = rois.unsqueeze(0).expand(n_points, n_boxes, n_reg_dim)
            # ic(rois.shape[1])
            ...
            box_feat = []
            mask = torch.zeros(n_boxes).bool().to(points.device)
    ...
    def bbox_select_train(self, preds_bboxes, gt_bboxes_3d):
        ...
        for bs in range(len(preds_bboxes)):
            ...
            if assign[0].shape[0] > 1:
                ...
                assign_0 = np.repeat(assign[0], 2)
                assign_0 = torch.from_numpy(assign_0).long().to(bbox_pred.device)
                second_pred_bboxes.append(bbox_pred[assign_0])
