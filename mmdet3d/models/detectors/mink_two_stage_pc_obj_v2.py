...
class MinkTwoStage3DDetectorPC2OBJ_V2(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 pretrained=None):
        super(MinkTwoStage3DDetectorPC2OBJ_V2, self).__init__(init_cfg)
    ...
    def forward_train(self, points, gt_bboxes_3d, gt_labels_3d, feat_3d, mask_chunk,  
                      gt_from_image_mask, image, intrinsics, extrinsics,  img_metas):  # 
        """Forward of training.
        ...
        batch_dict['semantic_feat'], batch_dict['semantic_feat_insseg'] = self.semantic_head.forward(batch_dict)
        ...
        losses.update(
            dict(distill_loss = distill_loss)
        )

        #########
        if  self.train_stage != '1st':
            batch_dict['gt_from_image_mask'] = gt_from_image_mask
            batch_dict['image'] = image
            batch_dict['intrinsics'] = intrinsics
            batch_dict['extrinsics'] = extrinsics
    ...
    def simple_test(self, points, img_metas, *args, **kwargs):
        ...
        #batch_dict['text_features'] = torch.load(self.text_features_file).to(torch.device('cuda:' + str(torch.cuda.device_count() - 1)))
        ...
        '''
        novel_calss_list = ['telephone', 'cardboard box', 'speaker', 'dustbin', 'computer monitor']
        for i, novel_class in enumerate(novel_calss_list):
            import os
            output_prefix = 'output_file/' + novel_class + '_bbox/'
            os.makedirs(output_prefix, exist_ok=True)
            bbox_novel = bbox_list[0][0][bbox_list[0][2]==i+17].tensor
            bbox_novel[:, 2] += bbox_novel[:, 5] / 2
            try:
                print(img_metas[0]['sample_idx'], bbox_list[0][1][bbox_list[0][2]==i+17][0], novel_class, bbox_novel.shape[0])
                ...
                if str(img_metas[0]['sample_idx']) == 'scene0274_00':
                    np.save('output_file/0274_'+novel_class+'.npy', bbox_novel.cpu().numpy())
            except:
                print(img_metas[0]['sample_idx'], [], novel_class, 0)
                
        if str(img_metas[0]['sample_idx']) == 'scene0274_00' and False:
            print(bbox_list[0][2])
            bbox_all = bbox_list[0][0].tensor
            bbox_all[:, 2] += bbox_all[:, 5] / 2
            print(bbox_all)
            write_oriented_bbox(bbox_all.cpu().numpy(), '/home/yifei/hyf/fcaf3d/output_file/all_bbox/' + str(img_metas[0]['sample_idx']) + '_all.ply')
        '''
