...
@DATASETS.register_module()
class ScanNetDatasetRec2SV(Custom3DDataset):
    r"""ScanNet Dataset for Detection Task.
    """
    CLASSES = ('bathtub', 'bed', 'bench', 'bookshelf', 'bottle', 'chair', 'cup', 'curtain', 'desk', 'door', 'dresser',
                 'keyboard', 'lamp', 'laptop', 'monitor', 'night_stand', 'plant', 'sofa', 'stool', 'table', 'toilet',
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode:
            ######img
            img_info = dict()
            input_dict['gt_from_image_mask'] = info['gt_from_image_mask']
            # img_info['img_paths'] = info['img_paths']
            # img_info['intrinsics'] = info['intrinsics']
            # axis_align_matrix = self._get_axis_align_matrix(info)
            # renew_extrinsics = []
            # for extrinsic in info['extrinsics']:
            #     renew_extrinsics.append(np.linalg.inv(axis_align_matrix @ extrinsic))
            # img_info['extrinsics'] = renew_extrinsics
            # input_dict['img_info'] = img_info
            imgs = []
            for i in range(len(info['img_paths'])):
                filename = info['img_paths'][i]
                if '999999' not in filename:
            ...
            input_dict['image'] = np.array(imgs)
            input_dict['intrinsics'] = info['intrinsics']
            axis_align_matrix = self._get_axis_align_matrix(info)
            renew_extrinsics = []
            for extrinsic in info['extrinsics']:
                if not np.all(np.equal(extrinsic, np.ones((4,4)) )):
                    renew_extrinsics.append(np.linalg.inv(axis_align_matrix @ extrinsic))
                else:
                    renew_extrinsics.append(extrinsic)
            input_dict['extrinsics'] = np.array(renew_extrinsics)
            # input_dict['img_info'] = img_info
            #############
            annos = self.get_ann_info(index)
            input_dict['ann_info'] = annos
...
class ScanNetDatasetOV3DetSV(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        input_dict['ann_info'] = annos
        if self.filter_empty_gt and ~(annos['gt_labels_3d'] != -1).any():
            print(self.filter_empty_gt, 'returning None')
            return None
        return input_dict
