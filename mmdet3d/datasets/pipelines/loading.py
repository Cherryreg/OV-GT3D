...
@PIPELINES.register_module()
class LoadMultiviewFeaturesPointsFromFile(object):
    def __init__(self,
                 coord_type,
                 load_dim=6,
                 use_dim=[0, 1, 2],
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.datapath_feat = datapath_feat
    def _load_points(self, pts_filename):
        """Private function to load point clouds data.
    ...
    def __call__(self, results):
        ...
        scene_name = pts_filename[-16:-4]
        n_occur = len(glob(os.path.join(self.datapath_feat,scene_name + '_*.pt')))
        if n_occur > 1:
                nn_occur = np.random.randint(n_occur)
        elif n_occur == 1:
                nn_occur = 0
        processed_data = torch.load(os.path.join(self.datapath_feat, scene_name+'_%d.pt'%(nn_occur)))
        feat_3d = processed_data['feat']
        mask_chunk = processed_data['mask_full']
        feat_3d_new = torch.zeros((points.shape[0], feat_3d.shape[1]), dtype=feat_3d.dtype)
        feat_3d_new[mask_chunk] = feat_3d
        feat_3d = feat_3d_new
        mask_ind = mask_chunk.nonzero(as_tuple=False)[:, 0]
        index1 = - torch.ones(mask_chunk.shape[0], dtype=int)
        index1[mask_ind] = mask_ind
        results['feat_3d'] = feat_3d
        results['mask_chunk'] = mask_chunk
        results['mask_ind'] = index1
        if self.shift_height:
            floor_height = np.percentile(points[:, 2], 0.99)
            height = points[:, 2] - floor_height
            points = np.concatenate(
...
@PIPELINES.register_module()
class LoadMultiviewFeaturesPointsFromFilesv(object):
    def __init__(self,
                 coord_type,
                 load_dim=6,
                 use_dim=[0, 1, 2],
    ...
    def __call__(self, results):
        ...
        points = points[:, self.use_dim]
        attribute_dims = None
        scene_name = pts_filename[-23:-4]
        #scene_name = scene_name[0:12]
        #print(scene_name)
        #n_occur = len(glob(os.path.join(self.datapath_feat,scene_name + '_*.pt')))
        n_occur = len(glob(os.path.join(self.datapath_feat,scene_name + '_*.pt')))
        #print(n_occur)
        ...
        try:
            ...
            print("ERROR", os.path.join(self.datapath_feat, scene_name+'_%d.pt'%(n_occur)))
            #print("ERROR", os.path.join(self.datapath_feat, scene_name+'_%d.pt'%(nn_occur)))
...
@PIPELINES.register_module()
class LoadImageFromFileOVDsv(object):
    def __init__(self, to_float32=False, color_type='unchanged'):
        self.to_float32 = to_float32
        self.color_type = color_type
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: The result dict containing the multi-view image data.
                ...
                - img_norm_cfg (dict): Normalization configuration of images.
        """
        filename = results['img_info']['image_path']
        ...
        img_calib = dict()
        img_calib['image'] = image
        img_calib['calib_K'] = results['img_info']['intrinsic']
        img_calib['calib_Rtilt'] = results['img_info']['pose']
        img_calib['image_size'] = image_size
        results['img_calib'] = img_calib   
        return results
